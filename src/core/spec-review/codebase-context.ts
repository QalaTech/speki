 ...
 */
async function listRelevantFiles(projectRoot: string): Promise<string[]> {
  const relevantFiles: string[] = [];
  const candidateDirs = new Set(['src', 'lib', 'app', 'packages', 'modules']);
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (candidateDirs.has(entry.name)) {
