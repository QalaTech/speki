...
import type { PRDData, SpecMetadata } from '../../../types/index.js';
...
describe('start command spec-partitioned state', () => {
  ...
  it('start_WithSpecFlag_ReadsFromSpecDir', async () => {
    ...
    writeFileSync(
      join(specDir, 'decompose_state.json'),
  ...
  it('start_UpdatesTaskProgressInSpecDir', async () => {
    ...
    if (loadedPrd) {
      ...
      writeFileSync(join(specDir, 'decompose_state.json'), JSON.stringify(loadedPrd, null, 2));
    }
