...
import { listSpecs, loadPRDForSpec } from '../../../core/spec-review/spec-metadata.js';
...
describe('tasks command spec-partitioned reading', () => {
  ...
  it('tasksList_WithSpecFlag_ReadsFromSpecDir', async () => {
    ...
    writeFileSync(join(specDir, 'decompose_state.json'), JSON.stringify(prd, null, 2));
  ...
  it('tasksList_WithMultipleSpecs_PromptsSelection', async () => {
    ...
    writeFileSync(join(spec1Dir, 'decompose_state.json'), JSON.stringify(prd1, null, 2));
    writeFileSync(join(spec2Dir, 'decompose_state.json'), JSON.stringify(prd2, null, 2));
