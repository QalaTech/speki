...
import type { ProjectStatus } from '@speki/core';
...
export const listCommand = new Command('list')
  ...
  .action(async (options) => {
    try {
      ...
      console.log(
        chalk.gray(
          `  ${'STATUS'.padEnd(12)} ${'NAME'.padEnd(maxNameLen)} ${'PATH'.padEnd(maxPathLen)} LAST ACTIVITY`
        )
      );
      ...
      for (const project of projects) {
        ...
        console.log(
          `  ${statusColor(statusText.padEnd(12))} ${project.name.padEnd(maxNameLen)} ${chalk.gray(
            project.path.padEnd(maxPathLen)
