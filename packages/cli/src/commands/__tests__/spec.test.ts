...
import type { SpecReviewResult, TimeoutInfo } from '@speki/core';
...
describe('handleGodSpec', () => {
  ...
  it('handleGodSpec_AcceptCreatesFiles', async () => {
    const { select } = await import('@inquirer/prompts');
    const { executeSplit } = await import('@speki/core');
...
describe('spec-partitioned review', () => {
  it('specReview_WithNewSpec_CreatesSpecDir', async () => {
    ...
    const { extractSpecId, getSpecDir } = await import('@speki/core');
  ...
  it('specReview_OnSuccess_TransitionsStatusToReviewed', async () => {
    ...
    const { transitionSpecStatus } = await import('@speki/core');
  ...
  it('specReview_WritesLogToSpecLogsDir', async () => {
    ...
    const { extractSpecId, getSpecLogsDir } = await import('@speki/core');
