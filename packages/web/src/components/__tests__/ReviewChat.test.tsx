...
describe('ReviewChat', () => {
  ...
  describe('ReviewChat_DisplaysHistory', () => {
    ...
    it('should display user messages with correct styling', () => {
      ...
      expect(messageEl).toHaveClass('self-end'); // User messages are right-aligned
    ...
    it('should display assistant messages with correct styling', () => {
      ...
      expect(messageEl).toBeInTheDocument();
      expect(messageEl).toHaveClass('self-start'); // Assistant messages are left-aligned
    ...
    it('should display message timestamps', () => {
      ...
      // Check that timestamp is rendered in the message
      const messageEl = screen.getByTestId('chat-message-user');
      // The component shows "You" or "Assistant" label and a timestamp span
      expect(messageEl).toHaveTextContent('You');
    });
  });
  ...
  describe('ReviewChat_SendsMessages', () => {
    ...
    it('should call onSendMessage when Enter pressed (without Shift)', async () => {
      ...
      await waitFor(() => {
        expect(onSendMessage).toHaveBeenCalledWith('Enter test', undefined, undefined);
      });
    });
  ...
  describe('ReviewChat_IncludesSelectionContext', () => {
    ...
    it('should include selection context when sending message', async () => {
      ...
      await waitFor(() => {
        expect(onSendMessage).toHaveBeenCalledWith('What about this?', 'Selected portion', undefined);
      });
    });
