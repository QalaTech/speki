...
describe('spec isolation', () => {
  ...
  it('isolation_SequentialDecompose_PreservesFirstSpecState', async () => {
    ...
    await writeFile(
      path.join(specDirA, 'tasks.json'),
      JSON.stringify(decomposeStateA, null, 2)
    );
    
    // Capture specA's tasks.json content before decomposing specB
    ...
    await writeFile(
      path.join(specDirB, 'tasks.json'),
      JSON.stringify(decomposeStateB, null, 2)
    );

    // Assert - specA's tasks.json should be unchanged
