...
describe('session-file', () => {
  ...
  it('getSessionPath_WithNestedSpecFile_ReturnsCorrectSessionPath', () => {
    ...
    // Act
    const result = getSessionPath(specFilePath, testDir);

    // Assert
    // Session files are now stored at .speki/specs/{specId}/session.json
    expect(result).toBe(
      join(testDir, '.speki', 'specs', 'my-feature-spec', 'session.json')
    );
  });
  ...
  it('loadSession_WithExistingSession_ReturnsSessionFile', async () => {
    ...
    // Session files are now at .speki/specs/{specId}/session.json
    const specDir = join(testDir, '.speki', 'specs', 'feature-spec');
    ...
    // Act
    const result = await loadSession(specFilePath, testDir);
  ...
  it('saveSession_WithValidSession_WritesJsonFile', async () => {
    ...
    // Assert
    // Session files are now at .speki/specs/{specId}/session.json
    const specDir = join(testDir, '.speki', 'specs', 'save-spec');
  ...
  it('saveSession_WithMissingDirectory_CreatesDirectory', async () => {
    ...
    // Assert
    // Session files are now at .speki/specs/{specId}/session.json
    const specDir = join(testDir, '.speki', 'specs', 'mkdir-spec');
    const savedContent = await readFile(
      join(specDir, 'session.json'),
      'utf-8'
    );
