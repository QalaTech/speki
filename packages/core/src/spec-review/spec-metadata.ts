...
export async function saveDecomposeStateForSpec(
  ...
  // Clean up stale queue references when the draft file changes
  // Load the PRD data to get current task IDs
  if (state.draftFile) {
    try {
      const prdContent = await readFile(state.draftFile, 'utf-8');
      const prd = JSON.parse(prdContent) as PRDData;
      await cleanupStaleQueueReferences(projectRoot, specId, prd.userStories || []);
    } catch {
      // If we can't read the PRD, don't clean up (might be in progress)
    }
  } else {
    // No draft file means no tasks - clean up all queue refs for this spec
    await cleanupStaleQueueReferences(projectRoot, specId, []);
  }
}

/**
 * Remove queue references for tasks that no longer exist in the decompose state.
 * This keeps the queue in sync when tasks are removed or spec is marked completed.
 */
async function cleanupStaleQueueReferences(
  projectRoot: string,
  specId: string,
  currentUserStories: Array<{ id: string }>
): Promise<void> {
  try {
    const queue = await loadTaskQueue(projectRoot);
    if (!queue) return;

    const currentTaskIds = new Set(currentUserStories.map(s => s.id));
    const originalLength = queue.queue.length;

    // Remove queue entries for this spec where the task no longer exists
    queue.queue = queue.queue.filter(ref => {
      if (ref.specId !== specId) return true; // Keep tasks from other specs
      return currentTaskIds.has(ref.taskId); // Only keep if task still exists
    });

    // Only save if we actually removed something
    if (queue.queue.length < originalLength) {
      await saveTaskQueue(projectRoot, queue);
    }
  } catch {
    // Don't fail the save operation if queue cleanup fails
  }
}
