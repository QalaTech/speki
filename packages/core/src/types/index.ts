...
export type DecomposeSseEvent =
  ...
  | SseEnvelope<'decompose/complete', { success: boolean; storyCount?: number; outputPath?: string; verdict?: string }>
  | SseEnvelope<'decompose/error', { success?: boolean; error?: string; details?: string }>;
...
export type SpecReviewSseEvent =
  ...
  | SseEnvelope<'spec-review/error', { sessionId: string; error: string }>;
...
/**
 * Reference to a task in a spec's tasks.json.
