...
export const statusCommand = new Command('status')
    ...
    try {
        ...
        if (options.json) {
            console.log(JSON.stringify({ config, status, decomposeState, prd }, null, 2));
            return;
        }
        ...
        if (status.currentIteration) {
            console.log(`  ${chalk.gray('Iteration:')} ${status.currentIteration}/${status.maxIterations || '?'}`);
        }
        ...
        if (decomposeState.verdict) {
            const verdictColor = decomposeState.verdict === 'PASS' ? chalk.green : chalk.red;
            console.log(`  ${chalk.gray('Verdict:')} ${verdictColor(decomposeState.verdict)}`);
        }
        console.log('');
        // PRD summary
        if (prd) {
            ...
            console.log(`  ${chalk.gray('Stories:')} ${completed}/${total} completed`);
