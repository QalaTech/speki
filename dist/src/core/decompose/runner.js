 ...
 */
async function getDecomposePrompt(project) {
    try {
 ...
 */
function getDefaultDecomposePrompt() {
 ...
 */
async function getNextUSNumber(project, freshStart) {
    if (freshStart) {
        return 1;
    }
 ...
 */
async function runClaudeDecompose(prompt, cwd, logPath) {
 ...
 */
async function reviseTasksWithFeedback(prdContent, currentTasks, feedback, project, attempt) {
 ...
 */
async function updateState(project, state, onProgress) {
 ...
 */
export async function runDecompose(project, options) {
    const { prdFile, branchName = 'main', language, outputName, freshStart = false, forceRedecompose = false, enablePeerReview = true, // Enabled by default - uses Codex for review
    maxReviewAttempts = 3, onProgress, } = options;
    ...
    if (forceRedecompose) {
        ...
        try {
            ...
            existingPrd = JSON.parse(existingContent);
    ...
    if (enablePeerReview) {
        ...
        while (reviewAttempt <= maxReviewAttempts) {
            ...
            if (verdict === 'PASS') {
                ...
                const revisedPrd = await reviseTasksWithFeedback(prdContent, prd, reviewResult.feedback, project, reviewAttempt);
                if (revisedPrd) {
                    prd = revisedPrd;
                    // Save revised tasks
                    await fs.writeFile(outputPath, JSON.stringify(prd, null, 2));
