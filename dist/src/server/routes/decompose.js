    ...
    try {
        ...
        for (const dir of searchDirs) {
            try {
                ...
                const mdFiles = dirFiles.filter((f) => f.endsWith('.md') && !f.startsWith('.'));
                files.push(...mdFiles.map((f) => ({
                    name: f,
...
router.get('/review-logs', async (req, res) => {
    try {
        ...
        const reviewLogs = files.filter(f => f.startsWith('peer_review_attempt_') && f.endsWith('.log'));
        const logs = await Promise.all(reviewLogs.map(async (file) => {
...
router.post('/start', async (req, res) => {
    try {
        const { prdFile, branchName, language, outputName, freshStart = false, forceRedecompose = false, } = req.body;
        if (!prdFile) {
...
router.post('/feedback', async (req, res) => {
    try {
        ...
        // Save feedback
        const { promises: fs } = await import('fs');
        await fs.writeFile(req.project.decomposeFeedbackPath, JSON.stringify({ feedback, timestamp: new Date().toISOString() }, null, 2));
        // Update state
        await req.project.saveDecomposeState({
            status: 'REVIEWING',
