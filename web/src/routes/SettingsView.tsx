  ...
  return (
    <div className="flex flex-col gap-6 p-6 overflow-y-auto h-full min-h-0">
      <div className="mb-2">
        ...
        <p className="m-0 text-sm text-base-content/60">Configure agent and model variants for different task stages</p>
      </div>
          ...
          {cliDetection && (
            <>
              {[
                ...
                <div
                  key={name}
                  className={`flex items-center gap-3 px-4 py-3 border-b border-base-100 last:border-b-0 ${
                    data.available ? '' : 'opacity-60'
                  }`}
                >
                  <span
                    className={`badge ${data.available ? 'badge-success' : 'badge-error'}`}
                  >
                    {data.available ? '\u2713' : '\u2717'}
                  </span>
                  <span className="flex-1 font-medium">{name}</span>
                  <span className={`font-mono text-sm ${data.available ? 'text-success' : 'text-base-content/50'}`}>
